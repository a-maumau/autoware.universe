...
CameraProjection::CameraProjection(
  ...
  image_w_ = camera_info_.width;
  image_h_ = camera_info_.height;
...
void CameraProjection::initialize(){
  if (unrectify_ && use_approximation_) {
    ...
    initializeCache();
    //calcRawImageProjectedPoint = [this](const cv::Point3d & point3d, Eigen::Vector2d & projected_point) {
    //  return this->calcRawImageProjectedPointApproximate(point3d, projected_point);
    //};
    //calcRawImageProjectedPoint =
    //  std::bind(&CameraProjection::calcRawImageProjectedPointApproximate, this, std::placeholders::_1, std::placeholders::_2);
  }
}
