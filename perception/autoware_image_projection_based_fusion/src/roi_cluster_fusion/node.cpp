  ...
  const sensor_msgs::msg::CameraInfo & camera_info, DetectedObjectsWithFeature & output_cluster_msg)
{
  ...
  {  // search matching ROI and update label
    ...
    for (const auto & feature_obj : input_roi_msg.feature_objects) {
      ...
      sensor_msgs::msg::RegionOfInterest image_roi = feature_obj.feature.roi;
      // condition for early stopping in ROI searching.
      const std::size_t image_roi_right_side_x = image_roi.x_offset + image_roi.width;
      const std::size_t image_roi_bottom_side_y = image_roi.y_offset + image_roi.height;
      ...
      for (const auto & cluster_map : m_cluster_roi) {
        ...
        // since cal_iou_by_mode has x, y mode, we need to check both x, y
        if (
